...
const Index = () => {
  ...
  const pitchers = useMemo(() => {
    return initialPitchers.map(pitcher => calculatePitcherStats(pitcher, outings)).sort((a, b) => a.name.localeCompare(b.name));
  }, [outings]);
  ...
  return <div className="min-h-screen bg-background">
      <Header viewMode={viewMode} onViewModeChange={setViewMode} onAddOuting={() => setShowOutingForm(true)} />

      <main className="container mx-auto px-4 py-6">
        {currentView === 'dashboard' && <div className="animate-slide-up">
            {/* Stats Summary */}
            <div className="mb-6">
              <h2 className="font-display text-2xl font-bold text-foreground mb-1">Roster</h2>
            ...
            {/* Desktop Table View */}
            {viewMode === 'table' && <div className="hidden md:block">
                <PitcherTable pitchers={pitchers} onPitcherClick={handlePitcherClick} />
              </div>}

            {/* Cards View (default on mobile, toggle on desktop) */}
            {(viewMode === 'cards' || window.innerWidth < 768) && <div className={viewMode === 'table' ? 'md:hidden' : ''}>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                  {pitchers.map(pitcher => <PitcherCard key={pitcher.id} pitcher={pitcher} onClick={() => handlePitcherClick(pitcher)} />)}
                </div>
              </div>}
          </div>}

        {currentView === 'detail' && selectedPitcher && <PitcherDetail pitcher={selectedPitcher} onBack={handleBackToDashboard} />}
      </main>
      ...
      <Sheet open={showOutingForm} onOpenChange={setShowOutingForm}>
        <SheetContent side="bottom" className="h-[90vh] overflow-y-auto bg-background border-border">
          <OutingForm pitchers={pitchers} onSubmit={handleAddOuting} onCancel={() => setShowOutingForm(false)} />
        </SheetContent>
      </Sheet>
    </div>;
};
